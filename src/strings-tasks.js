...
function formatTime(minutes, seconds) {
  const newMinutes = minutes < 10 ? String(minutes).padStart(2, '0') : minutes;
  const newSeconds = seconds < 10 ? String(seconds).padStart(2, '0') : seconds;
...
function findLongestWord(sentence) {
  return sentence.split(/\W+/).reduce((longest, current) => {
...
function reverseWords(str) {
  ...
  const newStr = str.split(/\W+/).map((item) => {
...
function extractNameFromTemplate(value) {
  return value.replaceAll('Hello,', '').replaceAll('!', '').trim();
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    encoded += index === -1 ? str[i] : output[index];
  }
  return encoded;
}
